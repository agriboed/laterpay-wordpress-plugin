...
class Post {
	...
	public static function getContentIDs( $post_id ) {
		...
		$subscriptions      = Subscription::get_tokenized_ids( $subscriptions_list );
	...
	public static function hasAccessToPost( WP_Post $post, $is_attachment = false, $main_post_id = null ) {
		...
		if ( apply_filters( 'laterpay_access_check_enabled', true ) && null !== \LaterPay\Core\Request::cookie( $token_name ) ) {
			...
			if ( ! $has_access ) {
				if ( array_key_exists( $post->ID, self::$access ) ) {
					...
					$result = Request::laterpayApiGetAccess(
						array_merge(
							array( $post->ID ),
