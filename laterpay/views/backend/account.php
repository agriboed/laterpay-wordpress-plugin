...
<div class="lp_page wp-core-ui">
	...
	<div class="lp_navigation">
		<a  href="<?php echo esc_url_raw( add_query_arg( array( 'page' => $laterpay['admin_menu']['account']['url'] ), admin_url( 'admin.php' ) ) ); ?>"
			...
			<?php
			if ( $laterpay['plugin_is_in_live_mode'] ) :
?>
...
?>
						<label class="lp_toggle__label lp_toggle__label-pass">
							<input type="checkbox"
								   ...
								   value="1"
								<?php
								if ( $laterpay['plugin_is_in_visible_test_mode'] ) {
									echo 'checked'; }
?>
>
...
?>
					<select id="lp_js_apiRegionSection" name="laterpay_region" class="lp_input">
						<option value="eu" 
						<?php
						if ( $laterpay['region'] === 'eu' ) {
							echo 'selected';}
?>
><?php echo laterpay_sanitize_output( __( 'Europe (EUR)', 'laterpay' ) ); ?></option>
						<option value="us" 
						<?php
						if ( $laterpay['region'] === 'us' ) {
							echo 'selected';}
?>
><?php echo laterpay_sanitize_output( __( 'United States (USD)', 'laterpay' ) ); ?></option>
					</select>
				</form>

				<p id="lp_js_regionNotice" 
				<?php
				if ( $laterpay['region'] === 'us' ) :
?>
