...
<?php if ( $laterpay['post_revenue_model'] === 'ppu' ): ?>
						lp_is-selected
<?php endif; ?>
<?php if ( in_array( $laterpay['post_price_type'], array(
				LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_PRICE,
				LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_DYNAMIC_PRICE,
			), true ) ) : ?>
...
<?php else : ?>
    <?php if ( $laterpay['post_revenue_model'] !== 'ppu' || $laterpay['price'] > $laterpay['currency']['ppu_max'] ): ?>
							lp_is-disabled
					<?php endif; ?>
<?php endif; ?> lp_tooltip"
                   data-tooltip="<?php echo esc_attr( __( 'Pay Later: users pay purchased content later',
				       'laterpay' ) ); ?>">
                <input type="radio"
			...
			<label class="lp_badge lp_badge--revenue-model lp_tooltip lp_mt-
					<?php if ( $laterpay['post_revenue_model'] === 'sis' ):?>
					lp_is-selected
					<?php endif; ?>
					<?php if ( in_array( $laterpay['post_price_type'], array( LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_PRICE, LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_DYNAMIC_PRICE ), true ) ) : ?>
                    <?php if ( $laterpay['price'] < $laterpay['currency']['sis_min'] ):?>
							lp_is-disabled
							<?php endif;?>
					<?php else : ?>
					<?php if ( $laterpay['post_revenue_model'] !== 'sis' ):?>
	 ...
	 --><div class="lp_layout__item lp_7/16">
			<input type="text"
					...
					name="post-price"
					value="<?php echo esc_attr( LaterPay\Helper\View::formatNumber( $laterpay['price'] ) ); ?>"
					placeholder="<?php echo esc_attr( __( '0.00', 'laterpay' ) ); ?>"
			<?php if ($laterpay['post_price_type'] !== LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_PRICE ):?>
					disabled
                   <?php endif;?>
            >
		</div><!-- layout works with display:inline-block; comments are there to suppress spaces
...
<?php
if ( in_array( $laterpay['post_price_type'], array( LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_DYNAMIC_PRICE, LaterPay\Helper\Pricing::TYPE_CATEGORY_DEFAULT_PRICE ), true ) ) {
	echo ' lp_is-expanded'; }
?>
">
	<ul id="lp_js_priceTypeButtonGroup" class="lp_price-type__list lp_clearfix">
		...
		<?php
		if ( in_array( $laterpay['post_price_type'], array( LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_PRICE, LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_DYNAMIC_PRICE ), true ) ) {
			echo 'lp_is-selected'; }
?>
">
			<a href="#"
		...
		<li class="lp_price-type__item
		<?php
		if ($laterpay['post_price_type'] === LaterPay\Helper\Pricing::TYPE_CATEGORY_DEFAULT_PRICE ) {
			echo 'lp_is-selected'; }
?>
...
">
		...
		<li class="lp_price-type__item
		<?php
		if ($laterpay['post_price_type'] === LaterPay\Helper\Pricing::TYPE_GLOBAL_DEFAULT_PRICE ) {
			echo 'lp_is-selected'; }
?>
...
">
			<a href="#"
				...
				data-price="<?php echo esc_attr( LaterPay\Helper\View::formatNumber( $laterpay['global_default_price'] ) ); ?>"
	...
	<div id="lp_js_priceTypeDetails" class="lp_price-type__details">
        <div id="lp_js_priceTypeDetailsIndividualPrice"
			<?php if ( $laterpay['post_price_type'] !== LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_DYNAMIC_PRICE ): ?>
                style="display:none;"
			<?php endif; ?>
        ...
        <div id="lp_js_priceTypeDetailsCategoryDefaultPrice"
			...
			<?php if ($laterpay['post_price_type'] !== LaterPay\Helper\Pricing::TYPE_CATEGORY_DEFAULT_PRICE ):?>
			style="display:none;"
            <?php endif; ?>>
			 ...
			 <ul class="lp_js_priceTypeDetailsCategoryDefaultPriceList lp_price-type-categorized__list">
				<?php if ( is_array( $laterpay['category_prices'] ) ) : ?>
					<?php foreach ( $laterpay['category_prices'] as $category ) : ?>
						...
						<?php if ( $category['category_id'] === $laterpay['post_default_category'] ) :?>
                            lp_is-selectedCategory
                        <?php endif; ?>">
							<a href="#"
								data-price="<?php echo esc_attr( LaterPay\Helper\View::formatNumber( $category['category_price'] ) ); ?>"
								data-revenue-model="<?php echo esc_attr( $category['revenue_model'] ); ?>">
								<span><?php echo esc_html( LaterPay\Helper\View::formatNumber( $category['category_price'] ) ); ?> <?php echo esc_html( $laterpay['currency']['code'] ); ?></span>
...
<?php if ($laterpay['post_price_type'] === LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_DYNAMIC_PRICE ) : ?>
	<?php if ($laterpay['post_status'] !== LaterPay\Helper\Pricing::STATUS_POST_PUBLISHED ) : ?>
		<dfn>
	...
	<?php else : ?>
		<a href="#"
			...
			data-icon="p">
            <?php echo esc_html( __( 'Restart dynamic pricing', 'laterpay' ) ); ?>
		</a>
	<?php endif; ?>
	<a href="#"
		...
		data-icon="e">
        <?php echo esc_html( __( 'Remove dynamic pricing', 'laterpay' ) ); ?>
	</a>
<?php else : ?>
	<a href="#"
		id="lp_js_toggleDynamicPricing"
		...
		data-icon="c"
		<?php if (0 !== strpos($laterpay['post_price_type'], LaterPay\Helper\Pricing::TYPE_INDIVIDUAL_PRICE)):?>
			style="display:none;"
       <?php endif;?>>
