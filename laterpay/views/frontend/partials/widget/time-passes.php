...
<div id="lp_js_timePassWidget" class="lp_time-pass-widget">
	<?php if ( $laterpay_widget['time_pass_introductory_text'] ) : ?>
		<p class="lp_time-pass__introductory-text">
            <?php echo wp_kses_post( $laterpay_widget['time_pass_introductory_text'] ); ?>
        </p>
	<?php endif; ?>

	<?php foreach ( $laterpay_widget['passes_list'] as $time_pass ) : ?>
		<?php echo $this->renderTimePass( $time_pass ); ?>
	<?php endforeach; ?>

	<?php if ( $laterpay_widget['subscriptions'] ) : ?>
		<?php echo $laterpay_widget['subscriptions']; ?>
	<?php endif; ?>
	...
	<?php if ( $laterpay_widget['has_vouchers'] ) : ?>
		<?php if ( $laterpay_widget['time_pass_call_to_action_text'] ) : ?>
			 <p class="lp_time-pass__call-to-action-text">
                 <?php echo wp_kses_post( $laterpay_widget['time_pass_call_to_action_text'] ); ?>
             </p>
		<?php endif; ?>
		...
		<div id="lp_js_voucherCodeWrapper" class="lp_redeem-code__wrapper lp_clearfix">
			...
			<p class="lp_redeem-code__input-hint">
                <?php echo esc_html( __( 'Code', 'laterpay' ) ); ?>
            </p>
			<div class="lp_js_voucherRedeemButton lp_redeem-code__button lp_button">
                <?php echo esc_html( __( 'Redeem', 'laterpay' ) ); ?>
            </div>
			<p class="lp_redeem-code__hint">
                <?php echo esc_html( __( 'Redeem Voucher >', 'laterpay' ) ); ?>
            </p>
		</div>
	<?php endif; ?>
</div>