...
class Event implements EventInterface
{
     ...
     */
    protected $propagationsStopped = false;

    /**
     * @var bool who has stopped event
     */
    protected $propagationsStoppedBy = '';
    ...
    {
        return $this->propagationsStopped;
    }
    ...
    public function setPropagationsStoppedBy($listener)
    {
        if (is_array($listener) && is_object($listener[0])) {
            $name = '[[object] (' . get_class($listener[0]) . ': {}),"';
            $name .= ( isset($listener[1]) ? $listener[1] : '__invoke' ) . '"]';
        } else {
            $name = (string) $listener;
        }
        $this->propagationsStoppedBy = $name;
    }
    ...
    public function getDebug()
    {
        return array(
            ...
            'propagation_stopped_by' => $this->propagationsStoppedBy,
