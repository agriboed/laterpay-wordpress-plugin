...
class Install extends ControllerAbstract
{
    ...
    public function checkRequirements()
    {
        ...
        $notices  = array();
        $template = __(
            '<p>LaterPay: Your server <strong>does not</strong> 
meet the minimum requirement of %1$s version %2$s or higher. You are running %3$s version %4$s.</p>',
            'laterpay'
        );
    ...
    public function maybeUpdateTimePassesTable()
    {
        ...
        if (version_compare($currentVersion, '0.9.11.4', '<')) {
            return;
        }
        ...
        $isDeletedFlagPresent = false;

        foreach ($columns as $column) {
            if (in_array($column->Field, $removedColumns, true)) {
                $isUpToDate = false;
            }
            if ($column->Field === 'is_deleted') {
                $isDeletedFlagPresent = true;
            }
        }
        ...
        $this->logger->info(
            ...
            array(
                ...
                'is_deleted_flag_present' => $isDeletedFlagPresent,
            )
        );

        // if the table needs an update
        if (! $isUpToDate) {
            $db->query('ALTER TABLE ' . $table . ' DROP title_color,
             DROP description_color, DROP background_color, DROP background_path;');
        }

        // if need to add is_deleted field
        if (! $isDeletedFlagPresent) {
    ...
    public function maybeAddIsInVisibleTestModeOption()
    {
        ...
        if (version_compare($currentVersion, '0.9.11', '<')) {
            return;
        }
    ...
    protected function maybeUpdateOptions()
    {
        ...
        if (version_compare($currentVersion, '0.9.8.1', '>=')) {
            delete_option('laterpay_plugin_is_activated');
        }

        if (version_compare($currentVersion, '0.9.14', '>=')) {
            delete_option('laterpay_access_logging_enabled');
        }

        if (version_compare($currentVersion, '0.9.25', '>')) {
    ...
    public function migratePricingPostMeta(Event $event)
    {
        ...
        if ($meta_key === 'laterpay_post_prices') {
            ...
            if (! empty($newMetaValues)) {
                add_post_meta($post_id, 'laterpay_post_prices', $newMetaValues, true);
            }
        }
