...
class Pricing extends TabAbstract
{
    ...
    protected function loadAssets()
    {
        ...
        $i18n = array(
            ...
            'confirmDeleteSubscription' => __(
                'Do you really want to discontinue this subscription?
                 If you delete it, it will continue to renew for users who have an active subscription 
                 until the user cancels it. Existing subscribers will still have access to the content 
                 in their subscription.  New users won\'t be able to buy the subscription anymore.
    ...
    protected function renderCategoryDefaultPrice(array $args = array())
    {
        ...
        foreach ($categoryPriceModel->getCategoriesWithDefinedPrice() as $cat) {
            ...
            $cat->sis_selected             = $cat->revenue_model === 'sis' ||
                                             ( ! $cat->revenue_model && $cat->category_price > $currency['ppu_max']);
    ...
    protected function renderTimePassesList(array $args = array())
    {
        ...
        foreach ($timePassModel->getActiveTimePasses() as $key => $pass) {
            ...
            foreach ($pass['vouchers'] as $code => $voucher) {
                $voucher['statistic']    = ! empty($vouchersStatistic[$pass['pass_id']][$code])
                    ? $vouchersStatistic[$pass['pass_id']][$code] : 0;
    ...
    public function help()
    {
        ...
        $screen->add_help_tab(
            array(
                ...
                'content' => __(
                    '
            <p>
                The plugin supports two currencies, depending on the region of your LaterPay merchant account:
                 EUR (€) for European merchant accounts, USD ($) for a U.S. merchant account.<br>
                ...
                For example, if your global default price is 0.10 EUR and you change the default currency to USD,
                 the global default
        ...
        $screen->add_help_tab(
            ...
            <p>
                ...
                With time passes, you can offer your users <strong>time-limited</strong> access to your content.
                 You can define, which content a time pass should cover and for which period of time it should 
                 be valid. A time pass can be valid for <strong>all LaterPay content</strong>
            </p>
            <ul>
            ...
            <p>
                The <strong>validity period</strong> of a time pass starts with the <strong>purchase</strong>
                 and is defined for a <strong>continuous</strong> use – i.e. it doesn\'t matter, if a user is
                  on your website during the entire validity period. After a time pass has expired, the access
                   to the covered content is automatically refused. Please note: Access to pages which are 
                   <strong>still open</strong> when a pass expires will be refused only after 
                   <strong>reloading</strong> the respective page. <strong>Any files</strong>
                    (images, documents, presentations...), that were downloaded during the validity
                     period, can still be used after the access has expired – but the user will 
                     <strong>not</strong> be able to <strong>download them </strong> without purchasing again.
            </p>
            <p>
                <strong>Deleting Time Passes</strong><br>
                If you <strong>delete</strong> a time pass, users who have bought this time pass 
                <strong>will still have access</strong> to the covered content. Deleted time passes
                 <strong>can\'t be restored</strong>.
            </p>
            <p>
                <strong>Time Passes and Individual Sales</strong><br>
                When a user purchases a time pass, he has access to all the content covered by this 
                pass during the validity period. Of course, you can still sell your content individually.<br>
                Example: A user has already purchased the post "New York – a Travel Report" for 0.29. 
                Now he purchases a Week Pass for the category "Travel Reports" for 0.99. The category
                 also contains the "New York" post. For one week, he can now read all posts in the 
                 category "Travel Reports" for a fixed price of 0.99. After this week, the access 
                 expires automatically. During the validity period, the user will not see any LaterPay
                  purchase buttons for posts in the category "Travel Reports". After the pass has expired,
                   the user will still have access to the post he had previously purchased individually.
            </p>',
                    'laterpay'
                ),
            )
        ...
        $screen->add_help_tab(
            array(
                ...
                'content' => __(
                    '
            <p>
                You can create any number of voucher codes for each time pass. A voucher code allows
                 one (or multiple) user(s) to purchase a time pass for a reduced price. 
                 A user can enter a voucher code right <strong>below the time passes</strong> 
                 by clicking <strong>"I have a voucher"</strong>. If the entered code is a valid 
                 voucher code, the price of the respective time pass will be reduced.<br>
                A voucher code can be used <strong>any number of times</strong> and is <strong>not linked</strong>
                 to a specific user. If you want to invalidate a time pass voucher code, you can simply delete it.<br>
                <strong>Deleting</strong> a voucher code will <strong>not affect</strong> the validity of
                 time passes which have already been purchased using this voucher code.
            </p>
            <p>
            Follow these steps to create a voucher code:
            </p>
            <ul>
                <li>Click the "Edit" icon next to the time pass for which you want to create a voucher code.,</li>
                <li>Enter a price next to \'Offer this time pass at a reduced price of\'. If you enter a price of 
                \'0.00\', anyone with this voucher code can purchase the respective time pass for 0.00.<br>
                    If you enter a price of e.g. \'0.20\', entering this voucher code will change the price of the 
                    respective time pass to 0.20.</li>
