...
class Admin extends Base {
	...
	public function addToAdminPanel() {
		...
		foreach ( $menu as $name => $page ) {
			...
			Hooks::addAction( 'load-' . $page_id, 'laterpay_load_' . $page_id );
			$help_action = isset( $page['help'] ) ? $page['help'] : array(
				$this,
				'help_' . $name,
			);
	...
	/**
	 * Constructor for class LaterPayController, processes the tabs in the
	 * plugin backend.
	 *
	 * @param string $tab
	 *
	...
	/**
	 * Load LaterPay stylesheet with LaterPay vector logo on all pages where
