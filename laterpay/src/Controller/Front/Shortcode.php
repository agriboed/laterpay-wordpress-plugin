...
class Shortcode extends ControllerAbstract
{
     ...
     *
     * Example:
     * [laterpay_box_wrapper][laterpay_premium_download target_post_title="Vocabulary list"]
     * [laterpay_premium_download target_post_title="Excercises"][/laterpay_box_wrapper]
     *
     * @param  EventInterface $event
     *
     * @return void
    ...
    public function ajaxGetPremiumShortcodeLink(EventInterface $event)
    {
        if (null === Request::get('action') ||
            sanitize_text_field(Request::get('action')) !== 'laterpay_get_premium_shortcode_link') {
        ...
        $currentPostID = absint(Request::get('post_id'));
        if (! get_post($currentPostID)) {
            throw new PostNotFound($currentPostID);
        }
        ...
        foreach ($IDs as $key => $id) {
            ...
            $access = Post::hasAccessToPost($post, $currentPostID);
            ...
            if ($access || ! $isPurchasable) {
                ...
                $buttonEvent->setArgument('post', $post);
                $buttonEvent->setArgument('current_post', $currentPostID);
                $buttonEvent->setArgument(
                    'attributes',
                    array(
                        'class' => 'lp_js_doPurchase lp_purchase-button lp_purchase-link--shortcode',
                    )
                );
                laterpay_event_dispatcher()->dispatch('laterpay_purchase_button', $buttonEvent);
                $htmlButton = $buttonEvent->getResult();
                if (empty($htmlButton)) {
                    $args = array(
                        'url' => get_permalink($post->ID),
                    );

                    $htmlButton = $this->getTextView(
                        'front/partials/post/shortcode-purchase-link',
                        array('_' => $args)
                    );
                }
            }
     ...
     *
     * @param array $timePasses list of time passes
     * @param null $link
     *
     * @return array
     */
    protected function addFreeCodesToPasses($timePasses, $link = null)
    {
        if (is_array($timePasses)) {
            foreach ($timePasses as $id => $timePass) {
                ...
                $timePass['url'] = TimePass::getLaterpayPurchaseLink(
                    $timePass['pass_id'],
                    $data,
                    true
                );
                $timePasses[$id] = $timePass;
            }
        }
     ...
     * - show: rendering options for the form as documented on
     * https://laterpay.net/developers/docs/inpage-api#GET/controls/links
    ...
    public function renderAccountLinks(EventInterface $event)
    {
        ...
        $linksEvent = new Event($view_args);
        $linksEvent->setEchoOutput(false);
        laterpay_event_dispatcher()->dispatch('laterpay_account_links', $linksEvent);

        $event->setResult($linksEvent->getResult());
    }
}
