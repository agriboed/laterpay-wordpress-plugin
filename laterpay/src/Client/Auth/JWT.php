...
class JWT
{
    ...
    public static function encode($secret, array $payload = array())
    {
        ...
        $data[] = self::base64URLEncode(hash_hmac(
            'sha256',
            implode('.', $data),
            $secret,
            true
    ...
    public static function decode($secret, $token)
    {
        ...
        $payload = static::jsonDecode(static::base64URLDecode($bodyb64));

        if (null === $payload) {
            throw new \UnexpectedValueException('Invalid claims encoding');
        }

        $sig = static::base64URLDecode($cryptob64);
        ...
        if (isset($payload->nbf) && $payload->nbf > ($timestamp + static::$leeway)) {
            throw new \LogicException(
                'Cannot handle token prior to ' . date(
                    \DateTime::ATOM,
                    $payload->nbf
                )
            );
    ...
    {
        return static::base64URLEncode(wp_json_encode($data));
    }
     ...
     *
     * @return mixed Object representation of JSON string
     */
