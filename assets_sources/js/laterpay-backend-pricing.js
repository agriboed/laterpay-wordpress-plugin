    ...
    function laterPayBackendPricing() {
        var $o = {
            ...
            formatSelect2ForEntity = function(data, container) {
                ...
                if (undefined !== data.access_category && undefined !== data.category_name) {
                    $($entity.fields.categoryId)
                        .val(data.access_category)
            ...
            populateEntityForm = function(type, $wrapper) {
                ...
                formatSelect2ForEntity(data, $entity.fields.scopeCategory);
                renderCategorySelect(
                    $wrapper,
            ...
            changeDurationOptions = function(type, $period, $form) {
                ...
                for(i = 1; i <= limit; i++) {
                    options.push($('<option>').val(i).text(i));
                }
            ...
            addVoucher = function(code, voucherData, $timePass) {
                var priceValue = voucherData.price ? voucherData.price : voucherData,
                    ...
                    voucher = $('<div class="lp_js_voucher lp_voucher">')
                        .data('code', code)
                        .hide()
                        .append($('<input type="hidden" name="voucher_code[]">').val(code))
                        .append($('<input type="hidden" name="voucher_price[]">').val(priceValue))
                        .append($('<span class="lp_voucher__code">').text(code))
                        .append($('<span class="lp_voucher__code-infos">').text(lpVars.i18n.voucherText + ' ' + price))
                        .append($('<input type="text" name="voucher_title[]" class="lp_input__title">').val(title))
                        .append($('<a href="#" class="lp_js_deleteVoucher lp_edit-link--bold" data-icon="g">'));
